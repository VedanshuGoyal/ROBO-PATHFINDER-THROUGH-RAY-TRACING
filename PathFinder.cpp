...
void PathFinder::onStart() {
	...
	cv::namedWindow("Observed Map", cv::WINDOW_AUTOSIZE);
	cv::namedWindow("Barrier map", cv::WINDOW_AUTOSIZE);
...
void PathFinder::onData(std::vector<std::pair<Ray, double> > rayMap) {
	...
	for (int i = 0; i < rayMap.size(); i++) {
		...
		if (dist > -1) {
			...
			if (targetX >= 0 && targetX < observedMap.cols
				...
				// dbg(observedMap(targetY, targetX));
	...
	cv::Mat display;
	cv::cvtColor(observedMap, display, cv::COLOR_GRAY2BGR);
...
void PathFinder::onDisplayForeground(cv::Mat display, double scale) {
	if (!output.isOpened()) {
		...
		output.open(filePath + ".avi", cv::VideoWriter::fourcc('M','J','P','G'), 20,
